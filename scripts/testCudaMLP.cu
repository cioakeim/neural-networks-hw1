...
int main(int argc,char* argv[]){
  ...
  DeviceMLP mlp=DeviceMLP(layer_sequence,INPUT_SIZE,
  ...
  for(int epoch=0;epoch<epochs;epoch++){
    ...
    J_train=mlp.runDeviceEpoch();
    cudaDeviceSynchronize();
    // Move to cpu and test if this thing actually works
    mlp.testDeviceModel(J_test,accuracy);
    std::cout<<"Device J_test: "<<J_test<<", Acc: "<<accuracy<<std::endl;
    mlp.deviceToHost();
    mlp.testModel(J_test,accuracy);
    std::cout<<"Host J_test: "<<J_test<<", Acc: "<<accuracy<<std::endl;
